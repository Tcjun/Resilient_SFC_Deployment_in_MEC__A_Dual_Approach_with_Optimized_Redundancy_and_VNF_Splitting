#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run workload-sharing sensitivity experiments for split active replicas."""
...
def tag_share(share: str) -> str:
    vals = parse_csv_floats(share)
    total = sum(vals)
    norm = [v / total for v in vals]
    return "_".join(tag_float(v) for v in norm)
...
def main() -> None:
    ...
    parser.add_argument("--out_root", default="paper_data/workload_sharing_sensitivity")
    parser.add_argument("--regions", default="Alabama,Arizona")
    parser.add_argument("--fixed_reliabilities", default="0.996,0.998,0.999")
    parser.add_argument("--seeds", default="1,2,3,4,5")
    parser.add_argument("--shares", default="0.5,0.5;0.6,0.4;0.7,0.3")
    ...
    parser.add_argument("--record_placement_trace", action="store_true")
    ...
    shares = parse_csv_strings(args.shares)
    rows: List[Dict] = []
    commands: List[str] = []
    ...
    for region in regions:
        for reliability in reliabilities:
            ...
            for seed in seeds:
                ...
                if args.record_placement_trace:
                    cmd.append("--record_placement_trace")
                ...
                for share in shares:
                    share_tag = tag_share(share)
                    out_dir = out_root / region / reliability_tag / f"DMDCA_share{share_tag}" / f"seed{seed}"
                    ...
                    metrics.update({
                        "method": "DMDCA",
                        "share": metrics.get("split_workload_shares", share),
                        "share_tag": share_tag,
                        "fixed_reliability": reliability,
                    })
                    rows.append(metrics)
    ...
    group_cols = ["region", "fixed_reliability", "method", "share", "share_tag"]
    ...
    for keys, group in raw.groupby(["region", "fixed_reliability", "seed"]):
        ...
        fairness_rows.append({
            ...
            "base_arrivals_consistent": bool(arrivals.nunique() == 1 and group["total_arrivals"].nunique() == 1),
        })
